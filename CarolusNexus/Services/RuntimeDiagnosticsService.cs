...
public static class RuntimeDiagnosticsService
{
    public static string BuildReport(NexusSettings settings)
    {
        ...
        sb.AppendLine(AiGovernanceUspService.BuildEvidenceModeReport(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[Process Mining Light]");
        sb.AppendLine(AiGovernanceUspService.BuildProcessMiningReport(settings));
        sb.AppendLine();
        sb.AppendLine("[Autonomy + Safe Mutation]");
        sb.AppendLine(AiGovernanceUspService.BuildAutonomyAndMutationReport(settings));
        sb.AppendLine();
        sb.AppendLine("[Pilot Readiness]");
        sb.AppendLine(PilotReadinessUspService.BuildPilotScorecard(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[Competitive USP]");
        sb.AppendLine(CompetitiveUspService.BuildBattlecard(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[AI AgentOps]");
        sb.AppendLine(AiAgentOpsUspService.BuildModelRouterReport(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[USP Studio]");
        sb.AppendLine(UspStudioService.BuildUspBacklog(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[AI Evaluation Lab]");
        sb.AppendLine(AiEvaluationLabService.BuildEvalLabReport(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[AI ROI + Opportunity]");
        sb.AppendLine(AiRoiOpportunityService.BuildRoiReport(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[AI Demo Orchestrator]");
        sb.AppendLine(AiDemoOrchestratorService.BuildClickPath(settings));
        sb.AppendLine();
        sb.AppendLine("[Pilot Proof + Release Readiness]");
        sb.AppendLine(ReleaseReadinessService.BuildReadinessReport(settings));
        sb.AppendLine();
        sb.AppendLine("[Enterprise AI Controls]");
        sb.AppendLine(AiPrivacyFirewallService.BuildFirewallReport(settings, ""));
        sb.AppendLine();
        sb.AppendLine(AiProcessMiningTimelineService.BuildTimeline());
        sb.AppendLine();
        sb.AppendLine("[Runtime Pilot Controls]");
        sb.AppendLine(AxFormMemoryService.BuildMemoryReport(settings));
        sb.AppendLine();
        sb.AppendLine(HumanApprovalCenterService.BuildReport());
        sb.AppendLine();
        sb.AppendLine(AiRiskSimulatorService.BuildRiskSimulation(settings, ""));
        sb.AppendLine();
        sb.AppendLine("[AX]");
